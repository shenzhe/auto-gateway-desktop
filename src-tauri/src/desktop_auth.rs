...
pub fn installation_id(app: &AppHandle) -> Result<String, String> {
    ...
    let temporary = directory.join(format!(".{INSTALLATION_ID_FILE}.{}.tmp", std::process::id()));
    let mut options = OpenOptions::new();
    options.write(true).create_new(true);
    #[cfg(unix)]
    options.mode(0o600);
    let mut file = options.open(&temporary).map_err(|error| format!("create installation ID file: {error}"))?;
...
pub async fn exchange_desktop_authorization(code: &str, code_verifier: &str, state: &str) -> Result<DesktopSession, String> {
    let response = desktop_http_client()?
        .post(format!("{AUTO_GATEWAY_SITE}/user/api/desktop/exchange"))
        .json(&DesktopExchangeRequest { code, code_verifier, state })
        .send()
        .await
...
pub fn save_desktop_session(app: &AppHandle, session: &DesktopSession) -> Result<(), String> {
    save_stored_state(app, &StoredDesktopState {
...
pub fn save_desktop_api_key(app: &AppHandle, api_key: &str) -> Result<(), String> {
    ...
    let mut stored = load_stored_state(app)?.ok_or_else(|| "desktop session is not stored".to_string())?;
...
pub async fn bootstrap_desktop_key(access_token: &str, installation_id: &str, rotate_existing: bool) -> Result<DesktopBootstrapKey, String> {
    let response = desktop_http_client()?
...
pub async fn create_desktop_console_ticket(access_token: &str) -> Result<DesktopConsoleTicket, String> {
    let response = desktop_http_client()?
        .post(format!("{AUTO_GATEWAY_SITE}/user/api/desktop/console-ticket"))
        .bearer_auth(access_token.trim())
        .send()
        .await
        .map_err(|error| format!("contact AUTO Gateway: {error}"))?;
...
pub async fn desktop_account_summary(access_token: &str) -> Result<DesktopAccountSummary, String> {
    let response = desktop_http_client()?
        ...
        .await
        .map_err(|error| format!("contact AUTO Gateway: {error}"))?;
    Ok(decode_response::<DesktopPurchaseResponse>(response, "read desktop account").await?.account)
}

async fn refresh_desktop_session(refresh_token: &str) -> Result<DesktopSession, SessionRefreshError> {
    let response = desktop_http_client()
    ...
    if status.is_success() {
        return response
            .json::<DesktopSession>()
            .await
            .map_err(|error| SessionRefreshError::Other(format!("decode AUTO Gateway response: {error}")));
    }
    ...
    Err(SessionRefreshError::Other(format!("refresh desktop session: {error}")))
}
...
fn load_stored_state(app: &AppHandle) -> Result<Option<StoredDesktopState>, String> {
    ...
    if path.exists() {
        ...
        set_private_file_permissions(&path)?;
        let value = fs::read(&path).map_err(|error| format!("read the encrypted desktop session file: {error}"))?;
...
fn load_legacy_stored_state() -> Result<Option<StoredDesktopState>, String> {
    ...
    let value = match entry.get_password() {
        ...
        Err(error) => return Err(format!("read the desktop session from the legacy operating-system credential store: {error}")),
    };
...
fn delete_stored_state(app: &AppHandle) -> Result<(), String> {
    ...
    if path.exists() {
        reject_symbolic_link(&path)?;
        fs::remove_file(&path).map_err(|error| format!("remove the encrypted desktop session file: {error}"))?;
    }
...
fn delete_legacy_stored_state() -> Result<(), String> {
    ...
    match entry.delete_credential() {
        ...
        Err(error) => Err(format!("remove the desktop session from the legacy operating-system credential store: {error}")),
    }
}
...
fn encrypt_stored_state_with_key(stored: &StoredDesktopState, mut key: [u8; 32]) -> Result<Vec<u8>, String> {
    let cipher = Aes256Gcm::new_from_slice(&key).map_err(|error| format!("prepare desktop session encryption: {error}"))?;
    key.zeroize();
    let nonce = Aes256Gcm::generate_nonce(&mut OsRng);
    let mut plaintext = serde_json::to_vec(stored).map_err(|error| format!("encode the desktop session: {error}"))?;
    let ciphertext = cipher
        .encrypt(&nonce, Payload { msg: &plaintext, aad: ENCRYPTION_CONTEXT })
...
fn decrypt_stored_state_with_key(value: &[u8], mut key: [u8; 32]) -> Result<StoredDesktopState, String> {
    ...
    if encrypted.format != ENCRYPTED_STATE_FORMAT || encrypted.version != ENCRYPTED_STATE_VERSION || encrypted.algorithm != "AES-256-GCM" {
    ...
    let cipher = Aes256Gcm::new_from_slice(&key).map_err(|error| format!("prepare desktop session decryption: {error}"))?;
    key.zeroize();
    let mut plaintext = cipher
        .decrypt(Nonce::from_slice(&nonce), Payload { msg: &ciphertext, aad: ENCRYPTION_CONTEXT })
        .map_err(|_| "decrypt the desktop session: the file is damaged or belongs to another computer user".to_string())?;
...
fn application_data_directory(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    fs::create_dir_all(&directory).map_err(|error| format!("create application data directory: {error}"))?;
    #[cfg(unix)]
...
fn write_private_file(path: &Path, value: &[u8]) -> Result<(), String> {
    let parent = path.parent().ok_or_else(|| "resolve the encrypted desktop session directory".to_string())?;
    let temporary = parent.join(format!(".{ENCRYPTED_STATE_FILE}.{}.tmp", Uuid::new_v4()));
    let mut options = OpenOptions::new();
    options.write(true).create_new(true);
    #[cfg(unix)]
    options.mode(0o600);
    let mut file = options.open(&temporary).map_err(|error| format!("create the encrypted desktop session file: {error}"))?;
    ...
    #[cfg(target_os = "windows")]
    if path.exists() {
        fs::remove_file(path).map_err(|error| format!("replace the encrypted desktop session file: {error}"))?;
    }
...
fn reject_symbolic_link(path: &Path) -> Result<(), String> {
    let metadata = fs::symlink_metadata(path).map_err(|error| format!("inspect private application data: {error}"))?;
...
fn platform_machine_binding() -> Result<String, String> {
    ...
    let platform_uuid = String::from_utf8_lossy(&output.stdout)
        ...
        .and_then(|line| line.split_once('=').map(|(_, value)| value.trim().trim_matches('\"').to_string()))
    ...
    let user_id = String::from_utf8_lossy(&user_output.stdout).trim().to_string();
...
async fn decode_response<T: for<'de> Deserialize<'de>>(response: reqwest::Response, action: &str) -> Result<T, String> {
    let status = response.status();
    if status.is_success() {
        return response.json::<T>().await.map_err(|error| format!("decode AUTO Gateway response: {error}"));
    }
    let error = response
        .json::<APIErrorResponse>()
        .await
...
#[cfg(test)]
mod tests {
    use super::{decrypt_stored_state_with_key, encrypt_stored_state_with_key, DesktopSession, DesktopUser, StoredDesktopState};
    ...
    fn encrypted_session_roundtrips_without_plaintext_secrets() {
        ...
        let encrypted = encrypt_stored_state_with_key(&stored, [7_u8; 32]).expect("encrypt the desktop session");
        let file_text = String::from_utf8(encrypted.clone()).expect("read the encrypted file as JSON");
        ...
        let decrypted = decrypt_stored_state_with_key(&encrypted, [7_u8; 32]).expect("decrypt the desktop session");
        assert_eq!(decrypted.session.token, stored.session.token);
        assert_eq!(decrypted.session.refresh_token, stored.session.refresh_token);
        assert_eq!(decrypted.api_key, stored.api_key);
    }

    #[test]
    fn encrypted_session_rejects_a_different_machine_key() {
        let encrypted = encrypt_stored_state_with_key(&stored_state(), [7_u8; 32]).expect("encrypt the desktop session");
